"""
app/api/field_extractor.py

USB Field Extractor - One-Click Logical Extraction (ported from NoteNext backend).
Connect Android via USB -> Extract chats, photos, contacts, browser -> Push to backend.

Run standalone: python -m app.api.field_extractor --extract --case KPC-2026-8941
Or via API: POST /api/field/extract
No root needed. Requires ADB (Android) installed. Falls back to MOCK if no device (for demo).
"""
...
def logical_extract(case_id="KPC-2026-8941"):
    """Do logical extraction (no root) - pulls key files via adb"""
    case_id = sanitize_case_id(case_id)
    info = device_info()
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    root = BACKUP_DIR.resolve()
    out_dir = (root / f"{case_id}_{ts}").resolve()
    # Defense in depth: output must stay inside the backup root
    try:
    ...
    if not info.get("connected"):
        # MOCK DEMO -- generate realistic fake extraction
        ...
        for rel, desc in mock_files:
            ...
            time.sleep(0.2)
        # save manifest
        manifest = {"case_id": case_id, "timestamp": ts, "device": info, "files": extracted, "mock": True, "hash_chain": hashlib.sha256(json.dumps(extracted).encode()).hexdigest()}
    ...
    # Try ADB backup for contacts/sms (creates .ab)
    log.append(f"  -> ADB backup (contacts/sms) ...")
    ...
    manifest = {"case_id": case_id, "timestamp": ts, "device": info, "files": extracted, "mock": False, "hash_chain": hashlib.sha256(json.dumps(extracted).encode()).hexdigest()}
    (out_dir / "manifest.json").write_text(json.dumps(manifest, indent=2))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--extract", action="store_true", help="Do extraction now")
    ap.add_argument("--case", default="KPC-2026-8941")
    ap.add_argument("--info", action="store_true", help="Only device info")
    args = ap.parse_args()
    if args.info:
        print(json.dumps(device_info(), indent=2))
    elif args.extract:
        res = logical_extract(args.case)
        print(json.dumps(res, indent=2))
        print(f"\nDone. Files at {res['out_dir']}")
    else:
        print("Usage: python -m app.api.field_extractor --info  OR  --extract --case KPC-2026-8941")